---
title: "Heuristic support for 310nm as an indicator of UVI"
author: "Shade, andover_motivation_story_v2.Rmd"
date: "9/10/2020"
output: pdf_document
---
...
without_outlier <- function(x){x[not_outlier(x)]}
...
```{r fit_outputs}
#     ----- FIT OUTPUTS ----
huber_uvnet <- read.csv("~/Downloads/huber_cmp.csv")  #TODO: include this calculation (from Fig 2)
...
hfit <- glm(uvi ~ i310, data=hpar[not_outlier(hpar$uvi), ])
hpar$uvi_err <- abs(hpar$uvi /( hfit$coefficients["i310"]*hspectra[, ind_310]/1000 + hfit$coefficients["(Intercept)"]) - 1)
...
savefig=TRUE  
ecbreaks <- c(0.02, 0.1, 0.30, 1, 1.5, 10000)
...
if(savefig){
png('fig4.png', height=480, width=480)
}

ggplot(hpar[rfilter, ], aes(x=ozone*37209.3, y=aerosol/2.302)) + 
  geom_point(aes(color=errf), size = 0.5, alpha = alpha) +
  xlab('Ozone (DU)') + ylab('Log(Aerosol Loss)') +
  geom_point(aes(x=340, y=0.83/2.302),color="black") +
  geom_vline(xintercept=93, linetype=2) + geom_vline(xintercept=220, linetype=3) +
  ylim(0,2) + xlim(0,750) + scale_color_discrete(labels=sclab) + labs(color="% error") +
  guides(colour = guide_legend(override.aes = list(size=4, alpha = 1))) +
  theme_bw() +
  theme(axis.title = element_text(size=18)) %>% 
  print  # figure saved as .png 480x480. .svg file is huge because of the 400,000 points.
