...
For the Andover paper, figure 1 is a review of the standard atmosphere focused on the UV and it's UVI.

The standard atmosphere, is ASTM G-173-03 and is well described at https://www.nrel.gov/grid/solar-resource/spectra-am1.5.html, where we downloaded the relevant tables (https://www.nrel.gov/grid/solar-resource/assets/data/astmg173.xls) on November 11, 2020, and then converted to .csv by saving the astmg173 tab as .csv using Microsoft Excel.
...
For our own sanity, we can compare this to the ISS solar spectral standard.
...
They are the same, but the ISS spectrum is reported with finer spectral resolution.
...
par(fig=c(0.5, 1, .5,1), new = T)
plot(o3$wavelength[o3$wavelength>305 & o3$wavelength<400], 1e22 * o3$O3_273[o3$wavelength>305 & o3$wavelength<400], type="l", col="blue", xlab="", ylab="", log="y", xlim=c(305,315), ylim=c(600,2000))
```
...
lines(irsimp$sdlam, irsimp$huber, col="black", lty=2)
```
...
Next up, the Erythema Action spectrum, or the health weighting of each wavelength.

```{r eas}
plot(irsimp$sdlam, eas(irsimp$sdlam), log="y", xlab="Wavlength (nm)", ylab="Relative skin sensitivity", type="l", lwd=2)
```

And the resulting standard skin damage spectrum is
...
Putting it all together in one tall figure
